...
final class Store: ObservableObject {
    @Published private(set) var items: [DuesheetItem] = []
    ...
    init() {
        let dir = FileManager.default.urls(for: .applicationSupportDirectory, in: .userDomainMask)[0]
            .appendingPathComponent("Duesheet", isDirectory: true)
    ...
    func add(title: String, note: String = "", value: Double = 0, date: Date = Date()) -> Bool {
        ...
        let item = DuesheetItem(title: title, note: note, value: value, date: date)
        items.insert(item, at: 0)
        save()
        return true
    }

    func update(_ item: DuesheetItem) {
    ...
    func delete(_ item: DuesheetItem) {
        items.removeAll { $0.id == item.id }
        save()
    }

    func toggleDone(_ item: DuesheetItem) {
        guard let idx = items.firstIndex(where: { $0.id == item.id }) else { return }
    ...
    private func load() {
        if let data = try? Data(contentsOf: fileURL),
           let decoded = try? JSONDecoder().decode([DuesheetItem].self, from: data) {
            items = decoded
        } else {
    ...
    static var seedData: [DuesheetItem] {
        let now = Date()
        let cal = Calendar.current
        return (0..<6).map { i in
            DuesheetItem(
                title: "Sample Assignment \(i + 1)",
